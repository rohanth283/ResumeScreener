...
async def get_applicants_search(
    ...
    # 1. Fetch query vector embedding from Gemini
    query_vector = await get_embedding(query)
    if not query_vector:
        raise HTTPException(status_code=500, detail="Failed to generate query embedding.")

    # 2. Get deduplicated candidates list for this user using SQL window CTE
    email_expr = sqlalchemy.case(
        (
            (models.Applicant.email == None) | 
            (func.trim(models.Applicant.email) == "") | 
    ...
    applicants = db.query(models.Applicant).join(
        subq, models.Applicant.id == subq.c.app_id
    ).filter(
        subq.c.rank == 1
    ).join(
    ...
    # 3. Calculate cosine similarity against candidates' stored resume embeddings
    results = []
    for app in applicants:
        similarity = 0.0
        if app.resume_embedding:
            try:
                v_cand = app.resume_embedding
                if isinstance(v_cand, str):
                    v_cand = json.loads(v_cand)
                similarity = cosine_similarity(query_vector, v_cand)
            except Exception:
                similarity = 0.0
        ...
        dto = schemas.ApplicantResponse.model_validate(app)
        dto.semantic_score = round(similarity * 100, 1)
        results.append(dto)

    # 4. Sort results descending by semantic relevance match
    results.sort(key=lambda x: x.semantic_score or 0.0, reverse=True)
    return results
